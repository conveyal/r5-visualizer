  ...
  renderLines () {
    ...
    if (searchState != null) {
      ...
      let thisRound = [...patternsUsedToReachAnyStopThisRound].map(pidx => {
        ...
        return <GeoJson data={data} color={'#f88'} weight={0.5} key={`pattern-${pidx}`} />
      })
  ...
  renderStops () {
    ...
    if (searchState != null && searchState.raptorState != null) {
      ...
      for (let i = 0; i < network.stops.length; i++) {
        ...
        else if (searchState.stopsTouchedThisMinuteFrequency[i]) color = '#5d5'
        else if (searchState.allStopsTouchedThisMinute[i]) color = '#77f'
