...
export default class SearchMap extends Component {
  ...
  renderLines () {
    ...
    if (searchState != null) {
      let out = []
      for (let i = 0; i < network.patterns.length; i++) {
        let color
        if (searchState.patternsExploredThisRound[i]) color = '#f88'
        else if (searchState.patternsExploredThisMinute[i]) color = '#99f'
        else continue // pattern not used this minute

        let data = {
          type: 'FeatureCollection',
          features: [{
            type: 'Feature',
            geometry: network.patterns[i],
            properties: {}
          }]
        }

        out.push(<GeoJson data={data} color={color} weight={0.5} key={`pattern-${i}`} />)
      }
