...
class Visualizer extends Component {
  render () {
    let { network, searchState, request, updateRequest } = this.props
    if (network == null) return <div>Loading . . .</div>

    return <div style={{ height: '100%', width: '100%' }}>
      <SearchMap network={network} searchState={searchState} request={request} style={{ height: '100%', width: '70%' }} updateRequest={updateRequest} />
